...
const bugIsDisabledOnMobile = window.matchMedia("(max-width: 760px)").matches;

if (bugIsDisabledOnMobile) {
    document.querySelector(".bug-layer")?.remove();
    document.querySelector(".bug-toggle")?.remove();
    document.querySelector(".site-footer__credits")?.remove();

    document
        .querySelector(".site-footer--with-credit")
        ?.classList.remove("site-footer--with-credit");
} else {
    ...
    function showBugTooltip() {
        ...
        if (
            ...
            window.innerHeight - tooltipMargin
        ) {
